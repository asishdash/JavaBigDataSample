package com.sparkSampleExample.sparkApi.Union;
...
//		    philDf.show(10);
	...
	public static Dataset<Row> buildDurhamParksDataFrame(SparkSession spark){
		Dataset<Row> df = spark.read().option("multiline", true)
			.json("src/main/resources/file/durham-parks.json");
	...
	private static Dataset<Row> buildPhilParksDataFrame(SparkSession spark) {
		Dataset<Row> df = spark.read().format("csv").option("multiline", true)
				.option("header", true)
				.load("src/main/resources/file/philadelphia_recreations_demo.csv");
		
//		df = df.filter(lower(df.col("USE_")).like("%park%"));
		df = df.filter("lower(USE_) like '%park%' ");
		df.show();
		df = df.withColumn("park_id", concat(lit("phil_"), df.col("AOBJECTID")))
