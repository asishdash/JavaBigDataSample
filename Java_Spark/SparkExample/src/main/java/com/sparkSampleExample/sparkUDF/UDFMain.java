package com.sparkSampleExample.sparkUDF;
...
public class UDFMain {
	...
	public static void main(String[] args) 
	{
		...
		Dataset<Row> dataset = spark.read().option("header", true).csv("src/main/resources/file/biglog.txt");
		
		SimpleDateFormat input = new SimpleDateFormat("MMMM");
		SimpleDateFormat output = new SimpleDateFormat("M");
		
		spark.udf().register("monthNumS", (String month) -> {
		...
		Dataset<Row> results = spark.sql
		  ("select level, date_format(datetime,'MMMM') as month, count(1) as total " + 
		   "from logging_table group by level, date_format(datetime,'MMMM') order by monthNumS(month), level");			
